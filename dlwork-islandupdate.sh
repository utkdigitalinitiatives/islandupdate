...
if [ -d "$UPDATE_EXEC_DIR/form_tables" ]; then
	...
	runuser -l islandora -c 'drush -v sql-cli < "$UPDATE_EXEC_DIR"/form_tables/xml_forms.sql'
	echo "Loading xml_form associations"
	runuser -l islandora -c 'drush -v sql-cli < "$UPDATE_EXEC_DIR"/form_tables/xml_form_builder_form_associations.sql'
